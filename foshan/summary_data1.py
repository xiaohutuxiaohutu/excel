...
def summary_data(path, dest_path):
    print(path)
    print(dest_path)
    ...
    # 第二个表单 从第三行开始去读  读取列数  第二列 实施事项名称 第五列 情形名称  第六列 材料名称  第8列 材料性质
    # 第10列 是否纸质必须  第11列 是否电子必须  第12列 是否支持容缺  第14列 是否批文批复 第15列  是否需要签章
    # 16列 原件数 17列 复印件数量
    sheet_0_col_index = [1, 2, 5]
    sheet_1_col_index = [1, 4, 5, 7, 9, 10, 11, 13, 14, 15, 15]
    sheet_0 = work_book.worksheets[0]

    sheet_0_rows = sheet_0.rows
    sheet_0_max_row = sheet_0.max_row
    # print('sheet_0:  max_row %i' % sheet_0_max_row)

    sheet_1 = work_book.worksheets[1]
    sheet_1_rows = sheet_1.rows
    sheet_1_max_row = sheet_1.max_row
    # print('sheet_0: max_row %i' % sheet_1_max_row)
    sheet0_row_value = []
    for sheet_0_row_index, row in enumerate(sheet_0_rows):
        # print(sheet_0_row_index)
        line = [col.value for col in row]  # 取值

        # print(line)
        if sheet_0_row_index == 0:
            # print(line)
            continue
        else:
            # print(line)
            temp_value = []
            for line_index in sheet_0_col_index:
                # print(line_index)
                new_value = line[line_index]
                temp_value.insert(len(temp_value), new_value)
            sheet0_row_value.insert(len(sheet0_row_value), temp_value)
    print(sheet0_row_value)
    '''
...
if __name__ == '__main__':
    input_file_path = read.open_multi_file_win('请选择需要合并的excel表', read.xls_xlsx_file_types)
    des_file_path = read.open_multi_file_win('请选择需要写入的目标模板文件', read.xlsx_file_types)
    for index, item in enumerate(input_file_path):
        print('读取第%i个文件:%s' % (index + 1, item))
        for index1, item1 in enumerate(des_file_path):
            summary_data(item, item1)
    # try:
    #     summary_data(item)
    # except:
